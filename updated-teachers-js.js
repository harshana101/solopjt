...
const translations = {
  ...
  // Function to prepare teacher data for HTML
  function createTeacherCard(teacher) {
    const card = document.createElement('div');
    card.className = 'teacher-card';
    card.setAttribute('data-name', teacher.name);
    card.setAttribute('data-subject', teacher.subject);
    card.setAttribute('data-categories', teacher.categories.join(','));
    
    // Add Sinhala translation attributes
    if (teacher.nameSi) card.setAttribute('data-name-si', teacher.nameSi);
    if (teacher.subjectSi) card.setAttribute('data-subject-si', teacher.subjectSi);
    
    // Create card content
    const currentLang = localStorage.getItem('preferredLanguage') || 'en';
    const displayName = currentLang === 'si' && teacher.nameSi ? teacher.nameSi : teacher.name;
    const displaySubject = currentLang === 'si' && teacher.subjectSi ? teacher.subjectSi : teacher.subject;
    
    card.innerHTML = `
      <div class="teacher-img">
        <img src="${teacher.image || 'assets/images/default-avatar.png'}" alt="${displayName}">
      </div>
      <div class="teacher-info">
        <h3 class="teacher-name">${displayName}</h3>
        <p class="teacher-position">${displaySubject}</p>
        <div class="categories">
          ${teacher.categories.map(cat => `<span class="category-tag">${currentLang === 'si' ? translations.si[getCategoryKey(cat)] || cat : cat}</span>`).join('')}
        </div>
        <button class="about-btn">${translations[currentLang].moreInfo}</button>
      </div>
    `;
    
    // Add event listener for the More Info button
    card.querySelector('.about-btn').addEventListener('click', function() {
      showTeacherDetails(teacher, currentLang);
    });
  ...
  function showTeacherDetails(teacher, lang) {
    // Create a modal with teacher details
    const modal = document.createElement('div');
    modal.className = 'teacher-modal';
    
    const displayName = lang === 'si' && teacher.nameSi ? teacher.nameSi : teacher.name;
    const displaySubject = lang === 'si' && teacher.subjectSi ? teacher.subjectSi : teacher.subject;
    const displayBio = lang === 'si' && teacher.bioSi ? teacher.bioSi : teacher.bio;
  ...
  function loadTeacherData() {
    ...
    return new Promise((resolve) => {
      // This is a sample teacher data. In a real application, fetch from an API or database.
      const sampleTeachers = [
        // Add more teachers as needed
        {
          ...
          bioSi: "වසර 10 ක පළපුරුද්දක් සහිත ගණිත ගුරුවරයෙකි."
        },
        {
          name: "Jane Smith",
          nameSi: "ජේන් ස්මිත්",
          subject: "Science",
          subjectSi: "විද්‍යාව",
          categories: ["O/L", "Scout"],
          image: "assets/images/teachers/teacher2.jpg",
          bio: "Science teacher specializing in Physics.",
          bioSi: "භෞතික විද්‍යාව විශේෂඥ විද්‍යා ගුරුවරයෙකි."
        }
      ];
      
      resolve(sampleTeachers);
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayRecentSearches() {
      ...
      if (!recentSearchesTitle) {
        ...
        title.id = 'recentSearchesTitle';
        title.textContent = translations[currentLang].recentSearches;
        recentSearches.appendChild(title);
      }
      
      // Remove all child elements except the title
      while (recentSearches.childNodes.length > 1) {
    ...
    // Load teacher data and populate the page
    const teacherContainer = document.getElementById('teacherContainer');
    if (teacherContainer) {
      loadTeacherData().then(teachers => {
